...
在 Flutter 中，AlertDialog 是一个小部件，它通知用户需要确认的情况。Flutter警告对话框包含一个显示在内容上方的可选标题和显示在内容下方的操作列表。
...
在AndroidStudio创建一个 Flutter 项目，用 **main.dart** 文件替换下面的代码。要显示警报，您必须调用 **showDialog()** 函数，该函数包含上下文和**项目生成器**函数。itemBuilder 函数返回一个**对象**，类型为**对话框，**AlertDialog。
