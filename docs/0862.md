...
我们可以将应用浏览器中的**定义为一个本地Cordova插件，主要用于为任何混合移动应用添加应用内浏览器。通过使用 InAppBrowser，我们可以从 Cordova 应用程序打开一个外部链接。它可以很容易地在Cordova应用程序中加载网页。用简单的语言，我们可以说这个插件在你的Cordova应用程序中加载了一个子浏览器，并加载了一个网页或第三方(不可信)内容。但是请注意，我们不能从这个插件访问Cordova应用程序接口，如**相机，电池状态，**等。为了方便用户，这个插件提供了一些默认的用户界面控件，比如**后退，前进，完成**。**

这是一个在[Cordova](https://www.javatpoint.com/apache-cordova)应用程序中的网络浏览器视图，可以通过调用**Cordova来显示。InAppBrowser.open()** 方法。
...
浏览器中的**方法在你的Cordova应用程序中加载一个子浏览器，但是这个方法不能访问Cordova应用程序接口，比如相机、电池状态等。因此，建议用户使用此方法加载第三方(不可信)内容。**
...
*   **_self:** 如果白名单中有网页的网址，该值将在Cordova网络视图中打开一个网页。否则，将在浏览器中打开**网页。**
